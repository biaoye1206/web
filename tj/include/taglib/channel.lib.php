...
function lib_channel(&$ctag,&$refObj)
{
    ...
    if($type=='top')
    {
        $sql = "SELECT id,description,typename,typeeng,typeicon,typecss,typepic,typebg,typedir,isdefault,ispart,defaultname,namerule2,moresite,siteurl,sitepath
        ...
        for($j=0; $j<$col; $j++)
        {
            ...
            if($row=$dsql->GetArray())
            {
                ...
                if( ($row['id']==$typeid || ($topid==$row['id'] && $type=='top') ) && $currentstyle!='' )
                {
					...
					$linkOkstr = str_replace("~typeeng~",$row['typeeng'],$linkOkstr);
                    $linkOkstr = str_replace("~typeicon~",$row['typeicon'],$linkOkstr);
					$linkOkstr = str_replace("~typecss~",$row['typecss'],$linkOkstr);
                    $linkOkstr = str_replace("~typepic~",$row['typepic'],$linkOkstr);
					$linkOkstr = str_replace("~typebg~",$row['typebg'],$linkOkstr);
                    $likeType .= $linkOkstr;
                }
                else
                {
