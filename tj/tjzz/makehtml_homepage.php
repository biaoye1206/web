...
else if($dopost=="make")
{
    ...
    if ($showmod == 1)
    {
		...
		$mposition = str_replace("..","../m",$position);
		$mhomeFile = str_replace("..","../m",$homeFile);
		$mtemplet = str_replace(".htm", "_m.htm", $templet);
		$mpv = new PartView();
		$mpv->SetTemplet($cfg_basedir.$cfg_templets_dir."/".$mtemplet);
		$mpv->SaveToHtml($mhomeFile);
        $pv->SetTemplet($cfg_basedir.$cfg_templets_dir."/".$templet);
        $pv->SaveToHtml($homeFile);
        echo "成功更新主页HTML：<br />电脑端主页 ".$homeFile."<br /><a href='{$position}' target='_blank'>浏览...</a><br />手机端主页 ".$mhomeFile."<br /><a href='{$mposition}' target='_blank'>浏览...</a><br />";
    } else { 
        // 动态浏览
